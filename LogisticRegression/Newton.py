# -*- coding: utf-8 -*-
"""
Created on Fri Dec 11 14:17:37 2015
...
def NewTheta(theta,x,y):
     ...
     for i in range(len(y)):
        J10+=H(theta,x[i])-y[i]
        H20+=x[i][0]*x[i][0]*H(theta,x[i])*(1-H(theta,x[i]))
        J11+=( H(theta,x[i])-y[i] )*x[i][1]
        H21+=x[i][1]*x[i][1]*H(theta,x[i])*(1-H(theta,x[i]))
        J12+=( H(theta,x[i])-y[i])*x[i][2]
        H22+=x[i][2]*x[i][2]*H(theta,x[i])*(1-H(theta,x[i]))
                
     theta[0]=theta[0]-J10/H20
     theta[1]=theta[1]-J11/H21
     theta[2]=theta[2]-J12/H22
     return theta



def Newton(theta,x,y,mean2,s2,xOld):
    step=1
    JthetaOld=J(theta,x,y)
    print("loop: %r  loss: %r"%(step,JthetaOld))
    #show(xOld,x,y,theta,step,mean2,s2)
    step=step+1
    theta=NewTheta(theta,x,y)
    JthetaNew=J(theta,x,y)
    print("loop: %r  loss: %r"%(step,JthetaNew))
    show(xOld,x,y,theta,step,mean2,s2)
    step=step+1
    Jdlt=JthetaOld-JthetaNew   
    while(Jdlt>=0.001 or Jdlt<=-0.001):
        JthetaOld=JthetaNew
        theta=NewTheta(theta,x,y)
        JthetaNew=J(theta,x,y)
        print("loop: %r  loss: %r"%(step,JthetaNew))
...
if __name__ == '__main__':
    ...
    s2=xTemp[2]
    theta=[0,0,0]
    theta= Newton(theta,x,y,mean2,s2,xOld)
    raw_input()
