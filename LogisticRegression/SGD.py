# -*- coding: utf-8 -*-
"""
Created on Fri Dec 11 13:03:41 2015
...
#        i=random.randint(0,79)
        #取到的随机样本为x[i],y[i]
        #更新theta
        theta[0]=theta[0]-a*((H(theta,x[i])-y[i])*x[i][0])
        theta[1]=theta[1]-a*((H(theta,x[i])-y[i])*x[i][1])
        theta[2]=theta[2]-a*((H(theta,x[i])-y[i])*x[i][2])  
    return theta



def SGD(theta,x,y,a,mean2,s2,xOld):
    step=1
...
if __name__ == '__main__':
    ...
    theta=[0,0,0]
    a=0.1
    theta=SGD(theta,x,y,a,mean2,s2,xOld)
    raw_input()
